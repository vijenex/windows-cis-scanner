...
"@
  ...
  if ($Formats -contains 'All' -or $Formats -contains 'PDF') {
    try {
      ...
      $pdfHtml = $htmlContent -replace '<style>.*?</style>', '<style>body{font-family:Arial,sans-serif;margin:20px;font-size:10px}.system-info{background:#f8f9fa;padding:10px;border:1px solid #ddd;margin:10px 0}.system-info h3{margin-top:0;font-size:14px}table{border-collapse:collapse;width:100%;font-size:9px}th,td{border:1px solid #000;padding:4px;text-align:left}th{background:#f0f0f0}.desc,.impact{max-width:200px;word-wrap:break-word}</style>'
      $tempHtml = Join-Path $env:TEMP "cis-temp-$(Get-Random).html"
      Set-Content -Path $tempHtml -Value $pdfHtml -Encoding UTF8
      
      # Try to use wkhtmltopdf if available, otherwise create instruction file
      $wkhtml = Get-Command wkhtmltopdf -ErrorAction SilentlyContinue
      if ($wkhtml) {
        & wkhtmltopdf --page-size A4 --orientation Portrait --margin-top 0.75in --margin-right 0.75in --margin-bottom 0.75in --margin-left 0.75in $tempHtml $pdf
        $outputs['PDF'] = $pdf
        Write-Host "PDF:  $pdf" -ForegroundColor Green
      } else {
        $pdfInstructions = Join-Path $OutDir 'PDF-Generation-Instructions.txt'
        Set-Content -Path $pdfInstructions -Value "To generate PDF report:`n1. Install wkhtmltopdf from https://wkhtmltopdf.org/downloads.html`n2. Add to PATH`n3. Re-run scanner with -OutputFormat PDF`n`nAlternatively, open $html in browser and print to PDF"
        Write-Host "PDF Instructions: $pdfInstructions (wkhtmltopdf not found)" -ForegroundColor Yellow
      }
  ...
  if ($Formats -contains 'All' -or $Formats -contains 'Word') {
    try {
      ...
      $selection.TypeText("Failed: $failed`n`n")
      ...
      $outputs['Word'] = $word
      Write-Host "Word: $word" -ForegroundColor Green
    } catch {
      $wordInstructions = Join-Path $OutDir 'Word-Generation-Instructions.txt'
      Set-Content -Path $wordInstructions -Value "To generate Word document:`n1. Ensure Microsoft Word is installed`n2. Re-run scanner with -OutputFormat Word`n`nAlternatively, open $html in browser and copy to Word`nError: $($_.Exception.Message)"
      Write-Host "Word Instructions: $wordInstructions (Word not available)" -ForegroundColor Yellow
    }
  }
